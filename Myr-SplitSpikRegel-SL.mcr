...
1.35 - 26.02.2020 - Added Kompl walls to the filter
...
/// <version  value="1.34" date="13.01.2020"></version>
...
/// OR - 1.34 - 13.01.2020 - SL openings above 2099 in heightRough won't add spikregel over openings
/// OR - 1.35 - 26.02.2020 - Added Kompl walls to the filter
/// </history>
...
String arSVerticalTypes[] = {"CL", "CP", "CT", "KL", "KP", "KT"};
...
if( _bOnElementConstructed || bManualInsert || _bOnDebug) {
	...
	//		reportNotice("\n"+d);
			...
			if( vyEl.dotProduct(ptSh - (ptElMid + vyEl * U(150))) > 0){
				if( abs(vxEl.dotProduct(ptSh - ptElEnd)) < U(100) || abs(vxEl.dotProduct(ptSh - ptElStart)) < U(150) ){
					sh.dbErase();
					ptSh.vis();
					continue;
				}
