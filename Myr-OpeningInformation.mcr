...
/// <version  value="1.10" date="13.05.2011"></version>
...
/// AS - 1.09 - 13.05.2011 - Use beamcode DRU instead of grade
/// AS - 1.10 - 13.05.2011 - Correct position of text
///</history>
...
for( int i=0;i<arSOpening.length();i++ ){
	...
	if( nDevidePointBy == 1 )
		ptOpening -= vyEl * U(450);
	else		
		ptOpening /= nDevidePointBy;
	
	Point3d ptOpCenPS = ptOpening; ptOpCenPS.transformBy(ms2ps);
	ptOpCenPS.vis(1);
	...
	else if (nInfoType==1) // Wall from Inside
	{
		...
		dpPlate.draw(sToDisplay, ptOpCenPS, _XW, _YW, 0, -2);
	}
}
