...
//---------------------------------Properties
//Stud width
PropDouble dStudWidth(0, U(45), T("Stud width"));
if( _Map.hasDouble("StudWidth") ){
	dStudWidth.set(_Map.getDouble("StudWidth"));
	_Map.removeAt("StudWidth",FALSE);
}
//Stud height
PropDouble dStudHeight(1,U(70), T("Stud height"));
if( _Map.hasDouble("StudHeight") ){
	dStudHeight.set(_Map.getDouble("StudHeight"));
	_Map.removeAt("StudHeight",FALSE);
}
//Stud length
PropDouble dStudLength(2,U(510), T("Stud length"));
if( _Map.hasDouble("StudLength") ){
	dStudLength.set(_Map.getDouble("StudLength"));
	_Map.removeAt("StudLength",FALSE);
}
//Height
PropString arSMountingHeight(0, "600;850", T("Mounting height (; separated list)"));

//Flooroffset
PropDouble dFloorOffset(3,U(0), T("Floor offset"));
if( _Map.hasDouble("FloorOffset") ){
	dFloorOffset.set(_Map.getDouble("FloorOffset"));
	_Map.removeAt("FloorOffset",FALSE);
}
//Information
PropString sInformation(1, "General text info", T("Information"));
...
if( _bOnInsert ){
	...
	_Pt0 = getPoint(T("Select a point for the height of the mounting stud"));
	_PtG.append(_Pt0);
	_Map.setInt("_Pt0 added", TRUE);
	while( TRUE ){
		PrPoint ssPt(T("Select a point for the height of the mounting stud"));
		if( ssPt.go() == _kOk ){
...
if( _PtG.length() > 0 ){
	arSMountingHeight.set("");
	for( int i=0;i<_PtG.length();i++ ){
		String sMountingHeight = arSMountingHeight;
		double dHeight = el.vecY().dotProduct(_PtG[i] - el.ptOrg());
		arSMountingHeight.set(sMountingHeight+dHeight+ ";");
	}
	_PtG.setLength(0);
}
...
//Project point to the side of zone 0 where the insertion point originaly is.
...
double arDMountingHeight[0];
int nMountingIndex = 0;
String sMountingHeight = arSMountingHeight.token(nMountingIndex);
while( sMountingHeight != "" ){
	double dMountingHeight = sMountingHeight.atof();
	if( dMountingHeight == 0 ){
		reportWarning(T("\nSpecified height is not a valid height!\nPlease fill in a ; separated list of heights!"));
		return;
	}
	arDMountingHeight.append(dMountingHeight);
	nMountingIndex++;
	sMountingHeight = arSMountingHeight.token(nMountingIndex);
}

//Only continue if the db is open for writing.
if( !_bOnWriteEnabled )return;
int nIndex = 0;
while( _Map.hasEntity("MountingStud"+nIndex) ){
	Entity ent = _Map.getEntity("MountingStud"+nIndex);
	ent.dbErase();
	_Map.removeAt("MountingStud"+nIndex, FALSE);
	nIndex++;
}

Point3d arPtExtraMountingStud[0];
int nNrOfMountingStudsCreated = 0;
for( int i=0;i<arDMountingHeight.length();i++ ){
	double dMountingHeight = arDMountingHeight[i];
	
	Point3d ptMountingStud = _Pt0 + el.vecY() * dMountingHeight;
		
	//find left and right beam to stretch the block to.
	Line lnBlock(ptMountingStud, el.vecX());
	double dLeft;
	int bLeftSet = FALSE;
	Beam bmLeft;
	double dRight;
	...
	for( int i=0;i<arBm.length();i++ ){
		...
		if( (el.vecY().dotProduct(ptIntersect - ptBmMin) * el.vecY().dotProduct(ptIntersect - ptBmMax)) < 0 ){
			double dDist = el.vecX().dotProduct(ptIntersect - ptMountingStud);
	...
	if( !bLeftSet ){
		reportWarning(T("\nBeam on left is not found"));
		return;
	}
	//Check if right beam is found			
	if( !bRightSet ){
		reportWarning(T("\nBeam on right is not found"));
		return;
	}
	Point3d ptLeft = ptMountingStud - el.vecX() * .5 * dStudLength;
	if( el.vecX().dotProduct((bmLeft.ptCen() + el.vecX() * .5 * bmLeft.dD(el.vecX())) - ptLeft) > dEps ){
		arPtExtraMountingStud.append(ptLeft - el.vecX() * bmLeft.dD(el.vecX()));
	}
	Point3d ptRight = ptMountingStud + el.vecX() * .5 * dStudLength;
	if( el.vecX().dotProduct(ptRight - (bmRight.ptCen() - el.vecX() * .5 * bmRight.dD(el.vecX()))) > dEps ){
		arPtExtraMountingStud.append(ptRight + el.vecX() * bmLeft.dD(el.vecX()));
	}

	//Create beam.
	Beam bmMountingStud;
	bmMountingStud.dbCreate(ptMountingStud, el.vecX(), el.vecY(), el.vecZ(), dStudLength, dStudHeight, dStudWidth, 0, 0, -nSide);
	bmMountingStud.stretchDynamicTo(bmLeft);
	bmMountingStud.stretchDynamicTo(bmRight);
	bmMountingStud.setColor(32);
	bmMountingStud.assignToElementGroup(el,TRUE,0,'Z');
	bmMountingStud.setName("Mounting stud");
	_Map.setEntity("MountingStud"+nNrOfMountingStudsCreated, bmMountingStud);
	nNrOfMountingStudsCreated++;
}

for( int i=0;i<arPtExtraMountingStud.length();i++ ){
	Point3d ptMountingStud = arPtExtraMountingStud[i];
